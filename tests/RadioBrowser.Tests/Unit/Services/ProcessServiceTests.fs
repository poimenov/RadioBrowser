...
open FsUnit.Xunit
open Moq
open RadioBrowser.ProcessService
...
module ProcessServiceTests =
    ...
    let ``ProcessService should implement IProcessService`` () =
        let mockLogger =
            TestFixtures.createMockLogger<RadioBrowser.Interfaces.IProcessService> ()

        let service = new ProcessService(mockLogger)

        service |> should be instanceOfType<RadioBrowser.Interfaces.IProcessService>
    ...
    let ``ProcessService.Run should accept valid command and arguments`` () =
        ...
        let service =
            new ProcessService(mockLogger) :> RadioBrowser.Interfaces.IProcessService

        let command = "notepad"
        let arguments = "test.txt"

        (fun () -> service.Run(command, arguments))
    ...
    let ``ProcessService should work with different shell commands`` (command: string) =
        ...
        (fun () -> service.Run(command, "/c echo test"))
        |> should not' (throw typeof<System.ArgumentException>)
    ...
    let ``ProcessService.Run with mock should verify call count`` () =
        ...
        let mockProcess = Mock<RadioBrowser.Interfaces.IProcessService>()
        let command = "test"
        let arguments = "arg"
